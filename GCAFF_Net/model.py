...
class GCASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        b3 = self.branch3(x)
        b4 = self.branch4(x)
        b5 = F.interpolate(self.branch5(x), size=size, mode='bilinear', align_corners=True)
...
class AFFM(nn.Module):
    ...
    def forward(self, res2, res3):
        ...
        res3_up = F.interpolate(res3, size=res2.shape[2:], mode='bilinear', align_corners=True)
...
class Model(nn.Module):
    ...
    def __init__(self, in_channels=2, num_classes=1):
        ...
        resnet = models.resnet101(weights=models.ResNet101_Weights.IMAGENET1K_V1, replace_stride_with_dilation=[False, False, True])
        ...
        self.affm = AFFM(low_channels=512, high_channels=1024, out_channels=256) # [cite: 185]
    ...
    def forward(self, x):
        ...
        # 2. Mid-level: AFFM
        feat_mid = self.affm(res2, res3) # [B, 256, H/8, W/8] (Output matches Res-2 size)
        ...
        feat_high_up = F.interpolate(feat_high, size=feat_mid.shape[2:], mode='bilinear', align_corners=True)
        ...
        fusion_1_up = F.interpolate(fusion_1, size=feat_low.shape[2:], mode='bilinear', align_corners=True)

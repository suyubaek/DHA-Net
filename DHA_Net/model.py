...
import math
from config import config  # Import config for main block
...
if __name__ == "__main__":
    ...
    # from config import config  # Import config # already imported at top
    ...
    model.eval()
    input_size = (1, 2, 256, 256)  # B, C, H, W
    dummy_input = torch.randn(*input_size).to(device)

    print("=" * 60)
    print(f"Model Efficiency Metrics: {config.get('model_name', 'DHA_Net')}")
    print(f"Input Size: {input_size}")
    print("=" * 60)

    # 2. Theoretical Indicators: Params & FLOPs
    # Note: thop.profile returns MACs. FLOPs is roughly 2 * MACs for CONV/FC.
    # We will report both if possible, or stick to thop output as GFLOPs (common convention).
    macs, params = profile(model, inputs=(dummy_input,), verbose=False)
    flops = macs * 2  # Approximate FLOPs
    print(f"{'Theoretical':<20} | {'Metric':<15} | {'Value':<15}")
    print("-" * 60)
    print(f"{'':<20} | {'Params (M)':<15} | {params / 1e6:.2f}")
    print(f"{'':<20} | {'FLOPs (G)':<15} | {flops / 1e9:.2f}")
    print(f"{'':<20} | {'MACs (G)':<15} | {macs / 1e9:.2f}")

    # 3. Hardware Indicators: Estimated MAC (Memory Access Cost)
    # Estimate total memory R/W: Sum of (Input + Output + Weight) for each layer
    # This is a rough estimation.
    def estimate_memory_access(model, input_tensor):
        total_mem_rw = 0

        def hook(module, input, output):
            nonlocal total_mem_rw
            # Input Read
            for i in input:
                if isinstance(i, torch.Tensor):
                    total_mem_rw += i.numel() * i.element_size()
            # Output Write
            if isinstance(output, torch.Tensor):
                total_mem_rw += output.numel() * output.element_size()
            # Weight Read
            for p in module.parameters():
                total_mem_rw += p.numel() * p.element_size()
    ...
    mem_rw_bytes = estimate_memory_access(model, dummy_input)
    print("-" * 60)
    print(f"{'Hardware':<20} | {'Mem Access (MB)':<15} | {mem_rw_bytes / 1e6:.2f}")

    # 4. Speed Indicators: Latency & Throughput
    # Warmup
    for _ in range(50):
        _ = model(dummy_input)
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.synchronize()
        avg_latency_ms = start.elapsed_time(end) / runs
    else:
        t0 = time.time()
        for _ in range(runs):
            _ = model(dummy_input)
        avg_latency_ms = (time.time() - t0) * 1000 / runs

    print("-" * 60)
    print(f"{'Speed (B=1)':<20} | {'Latency (ms)':<15} | {avg_latency_ms:.2f}")

    # Throughput (B=8 or similar for max saturation)
    batch_size_throughput = 8
    input_throughput = torch.randn(batch_size_throughput, 2, 256, 256).to(device)
    runs_throughput = 50

    # Warmup
    for _ in range(10):
        _ = model(input_throughput)

    if torch.cuda.is_available():
        torch.cuda.synchronize()
        start = time.time()
        for _ in range(runs_throughput):
            _ = model(input_throughput)
        torch.cuda.synchronize()
    ...
    fps = (runs_throughput * batch_size_throughput) / total_time

    # Calculate km2/s assumptions
    # 256x256 pixels, GSD=10m => 2.56km x 2.56km = 6.5536 km2 per image
    gsd_m = 10
    area_per_img_km2 = (256 * gsd_m / 1000) ** 2
    throughput_km2_s = fps * area_per_img_km2
